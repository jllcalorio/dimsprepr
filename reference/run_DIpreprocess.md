...
by orchestrating the individual `run_*` functions of dimsprepr. Steps
...
-\> quality filtering -\> replicate-merging branch is then repeated for
every normalization x transformation pair. The return value in this case
is a plain `list` (class `"run_DIpreprocess_multi"`) whose elements are
named `<normalize>_<transform>` and each hold a standard
`run_DIpreprocess` object.
